...
<title>K-9 Security Report — {_esc(meta.get('cidr',''))}</title>
<style>
  ...
  <h1>K-9 Network Security Report</h1>
  <div class="sub">Network <b>{_esc(meta.get('cidr','?'))}</b> · generated {_esc(meta.get('timestamp','')) }
    · {total} devices · authorised scope: {scope_txt} · K-9 v{__version__}</div>
  <div class="summary">
  ...
  {''.join(cards)}
  <footer>Generated by K-9 · quiet (passive-leaning) audit · for authorised assessment only · data stays on the operator's machine</footer>
</div></body></html>"""
